...
var import_compiler = require("imba/compiler");

// src/utils/hash.ts
var crypto = __toESM(require("crypto"), 1), hashes = /* @__PURE__ */ Object.create(null), hash_length = 12;
...
// src/utils/log.ts
var import_colors = require("kleur/colors"), import_debug = __toESM(require("debug"), 1), levels = ["debug", "info", "warn", "error", "silent"], prefix = "vite-plugin-imba", loggers = {
  debug: {
...
var scriptLangRE = /<script [^>]*lang=["']?([^"' >]+)["']?[^>]*>/, _createCompileImba = (makeHot) => async function(imbaRequest, code, options) {
  ...
  let compiled = (0, import_compiler.compile)(finalCode, finalCompileOptions);
...
// src/svg-plugin.imba
var import_fs = require("fs"), import_compiler2 = require("imba/compiler");
function compileΞsvg(body, url2) {
  let js, parsed = (0, import_compiler2.parseAsset)({ body });
  ...
  return {
    ...
    transform: async function(source, id, opts) {
      if (id.endsWith(".svg")) {
        ...
        if (!result2) {
          let body = (0, import_fs.readFileSync)(id, "utf-8");
...
// src/utils/id.ts
var import_pluginutils = require("@rollup/pluginutils"), import_vite = require("vite"), fs2 = __toESM(require("fs"), 1), VITE_FS_PREFIX = "/@fs/", IS_WINDOWS = process.platform === "win32";
function splitId(id) {
...
function buildFilter(include, exclude, extensions) {
  let rollupFilter = (0, import_pluginutils.createFilter)(include, exclude);
  return (filename) => rollupFilter(filename) && extensions.some((ext) => filename.endsWith(ext));
}
function buildIdParser(options) {
  let { include, exclude, extensions, root } = options, normalizedRoot = (0, import_vite.normalizePath)(root), filter = buildFilter(include, exclude, extensions);
...
var import_module = require("module"), import_path = __toESM(require("path"), 1), import_fs2 = __toESM(require("fs"), 1), import_url = require("url");
var import_meta = {}, esmRequire, knownImbaConfigNames = [
  "imba.config.js",
  "imbaconfig.json",
  "imba.config.json"
...
async function loadImbaConfig(viteConfig, inlineOptions) {
  ...
  if (configFile) {
    ...
    if (configFile.endsWith(".json"))
      try {
        let result = await dynamicImportJSON(
          ...
          import_fs2.default.statSync(configFile).mtimeMs
        );
        if (result != null)
          return {
            ...result,
    ...
    if (!configFile.endsWith(".mjs"))
      try {
        let _require = import_meta.url ? esmRequire ?? (esmRequire = (0, import_module.createRequire)(import_meta.url)) : require;
...
function findConfigToLoad(viteConfig, inlineOptions) {
  ...
  if (inlineOptions != null && inlineOptions.configFile) {
    let abolutePath = import_path.default.isAbsolute(inlineOptions.configFile) ? inlineOptions.configFile : import_path.default.resolve(root, inlineOptions.configFile);
    if (!import_fs2.default.existsSync(abolutePath))
      throw new Error(`failed to find imba config file ${abolutePath}.`);
    return abolutePath;
  } else {
    let existingKnownConfigFiles = knownImbaConfigNames.map((candidate) => import_path.default.resolve(root, candidate)).filter((file) => import_fs2.default.existsSync(file));
...
var import_path2 = __toESM(require("path"), 1), import_fs3 = __toESM(require("fs"), 1), import_module2 = require("module");
function findRootImbaDependencies(root, cwdFallback = !0) {
  log.debug(`findImbaDependencies: searching imba dependencies in ${root}`);
  let pkgFile = import_path2.default.join(root, "package.json");
  if (!import_fs3.default.existsSync(pkgFile)) {
    if (cwdFallback) {
      let cwd = process.cwd();
      if (root !== cwd)
...
function getImbaDependencies(deps, pkgDir, path7 = []) {
  let result = [], localRequire = (0, import_module2.createRequire)(`${pkgDir}/package.json`), resolvedDeps = deps.map((dep) => resolveDependencyData(dep, localRequire)).filter(Boolean);
...
function resolveDependencyData(dep, localRequire) {
  try {
    ...
    return { dir: import_path2.default.dirname(localRequire.resolve(pkgJson)), pkg };
  } catch (e) {
    ...
    try {
      ...
      for (; dir; ) {
        ...
        if (pkg && pkg.name === dep)
          return { dir, pkg };
        let parent = import_path2.default.dirname(dir);
        if (parent === dir)
          break;
        dir = parent;
      }
...
function parsePkg(dir, silent = !1) {
  let pkgFile = import_path2.default.join(dir, "package.json");
  try {
    return JSON.parse(import_fs3.default.readFileSync(pkgFile, "utf-8"));
  } catch (e) {
...
function needsOptimization(dep, localRequire) {
  ...
  if (pkg.module || pkg.exports)
    return !1;
  if (pkg.main) {
    let entryExt = import_path2.default.extname(pkg.main);
...
var import_fs4 = require("fs"), import_compiler3 = require("imba/compiler");
...
async function buildMagicString(from, to, options) {
  ...
  let m = new import_magic_string.default(from, options), pos = 0;
...
function createInjectScopeEverythingRulePreprocessorGroup() {
  return {
    style({ content, filename }) {
      let s = new import_magic_string2.default(content);
      return s.append(" *{}"), {
        code: s.toString(),
...
// src/utils/options.ts
var import_deepmerge = __toESM(require("deepmerge"), 1), allowedPluginOptions = /* @__PURE__ */ new Set([
  "include",
  "exclude",
  "emitCss",
  "hot",
...
function resolveViteRoot(viteConfig) {
  return (0, import_vite2.normalizePath)(viteConfig.root ? import_path4.default.resolve(viteConfig.root) : process.cwd());
}
...
function buildOptimizeDepsForImba(imbaDeps, options, optimizeDeps) {
  ...
  if (log.debug(`automatically excluding found imba dependencies: ${imbaDepsToExclude.join(", ")}`), exclude.push(...imbaDepsToExclude.filter((x) => !isExcluded(x))), options.disableDependencyReinclusion !== !0) {
    ...
    let transitiveDepsToInclude = imbaDeps.filter((dep) => !disabledReinclusions.includes(dep.name) && isExcluded(dep.name)).flatMap((dep) => {
      let localRequire = (0, import_module3.createRequire)(`${dep.dir}/package.json`);
...
var import_path5 = __toESM(require("path"), 1), import_module4 = require("module");
function resolveViaPackageJsonImba(importee, importer, cache) {
  if (importer && isBareImport(importee) && !isNodeInternal(importee) && !is_common_without_imba_field(importee)) {
    ...
    if (cached)
      return cached;
    let localRequire = (0, import_module4.createRequire)(importer), pkgData = resolveDependencyData(importee, localRequire);
    if (pkgData) {
      let { pkg, dir } = pkgData;
      if (pkg.imba) {
        let result = import_path5.default.resolve(dir, pkg.imba);
...
function setupWatchers(options, cache, requestParser) {
  ...
  let { watcher, ws } = server, { root, server: serverConfig } = server.config, emitChangeEventOnDependants = (filename) => {
    cache.getDependants(filename).forEach((dependant) => {
      import_fs5.default.existsSync(dependant) && (log.debug(
  ...
  if (imbaConfigFile !== !1) {
    let possibleImbaConfigs = knownImbaConfigNames.map((cfg) => import_path6.default.join(root, cfg)), restartOnConfigAdd = (filename) => {
...
function ensureWatchedFile(watcher, file, root) {
  file && !file.startsWith(root + "/") && !file.includes("\0") && import_fs5.default.existsSync(file) && watcher.add(import_path6.default.resolve(file));
}
